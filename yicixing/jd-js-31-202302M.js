    ...
    if(resp.url.indexOf('/statement/list?') >= 0){
      ...
      for(var i = 0; i < data.length; i++){
        ...
        if(i == 0){
            ...
            row.cheatNum = 2171;
            row.cheatRate = 5.4;
            row.statementCost = 225734.63;
            row.avgActivePrice = 5.94;
        }
      }
      ...
      var data = json.data;
        var row = data;
        row.activeNum = 40167;
        row.activeNumPer = -24.99;
        row.normalNum = 37996;
        row.normalNumPer = -25.59;
        row.cheatRate = 5.4;
        row.cheatRatePer = 16.38;
        row.statementCost = 225734.63;
        row.statementCostPer = -44.11;
        row.avgActivePrice = 5.94;
        row.avgActivePricePer = -24.91;
      console.log('json', json);
      return JSON.stringify(json);
    }
    return content;
