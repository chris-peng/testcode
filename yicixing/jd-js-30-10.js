    ...
    if(resp.url.indexOf('/statement/list?') >= 0){
      ...
      for(var i = 0; i < data.length; i++){
        ...
        if(i == 0){
            ...
            row.cheatNum = 14630;
            row.cheatRate = 72.26;
            row.statementCost = 30174.42;
            row.avgActivePrice = 5.37;
        }
      }
      ...
      var data = json.data;
        var row = data;
        row.activeNum = 20247;
        row.activeNumPer = -3.61;
        row.normalNum = 5617;
        row.normalNumPer = -50.08;
        row.cheatRate = 72.26;
        row.cheatRatePer = 55.6;
        row.statementCost = 30174.42;
        row.statementCostPer = -67.92;
        row.avgActivePrice = 5.37;
        row.avgActivePricePer = -35.74;
      console.log('json', json);
      return JSON.stringify(json);
    }
    return content;
